...
impl Cache {
    pub fn cache(cache: &RwLock<Cache>) {
        ...
        let ts: Vec<TSPackage> = reqwest::blocking::get("https://thunderstore.io/api/v1/package/").unwrap().json().unwrap();

        c.packages.shrink_to(ts.len());

        for package in ts {
            c.packages.insert(package.full_name.clone().to_lowercase(), package.into());
        }
    }
    ...
    pub async fn enable_auto_update(cache: Arc<RwLock<Cache>>, timeout: Duration) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                tokio::task::spawn_blocking(move || Cache::cache(&arc_inner)).await.unwrap();
            }
        });
    ...
    pub fn search(&self, q: Option<String>) -> SearchResult {
...
impl From<&NugetPackage> for SearchItem {
    fn from(pkg: &NugetPackage) -> Self {
        Self {
            ...
            registration: format!("{}/nuget/v3/package/{}/index.json", crate::BASE_URL, pkg.items[0].full_name)
        }
    }
}
